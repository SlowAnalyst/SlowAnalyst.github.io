...
여기서 확장 연산은 선형 연산이므로 X1 ^ X2 = 0x02인 입력 X1, X2에 대해 다음이
성립한다.
...
```
R4 ^ R_4 = F(L4, K4) ^ F(L_4, K4) = 0000 0010 Prob .75
				(L4 ^ L_4 = 0x02 이므로) --- (16)
```
...
찾았을 때 비로소 평문을 복구할 수 있을 것이다. 정확한 키를 찾을 때까지 시도해야하는 
조사 횟수의 기대값은 2^12의 절반인 2^11이라고 할 수 있다. 그러므로 전체 작업량의
